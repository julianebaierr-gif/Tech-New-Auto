                ...
                try:
                    with open(os.path.join(POSTS_DIR, f), "r", encoding="utf-8") as pf:
                        ...
                        posts.append({
                            ...
                            "target_keyword": d.get("target_keyword", "").strip(),
...
def main():
    ...
    def clean_content(html_text):
        ...
        # Automated Contextual Natural Internal Linking to Existing Relevant Posts (4 to 6 links)
        # Requirement: Anchor on specific technical keywords or concise natural references, NOT dumping entire full titles
        ...
        if existing_pool and len(paras := list(re.finditer(r'<p>([\s\S]*?)</p>', text))) >= 6:
            ...
            # Determine target internal link count (4 to 6 links)
            import random
            ...
            if k >= 3 and num_paras >= (k + 2):
                ...
                for i, p_idx in enumerate(sorted(unique_indices)[:len(chosen_internal_links)]):
                    ...
                    cand_kw = target_post.get("target_keyword", "").strip()
                    if not cand_kw and target_post.get("tags"):
                        cand_kw = target_post["tags"][0]
                    if not cand_kw:
                        # Extract core subject from title (strip stop words)
                        words = [w for w in target_post["title"].split() if w.lower() not in ["the", "a", "an", "and", "or", "for", "to", "in", "of", "with", "how", "what", "top", "best"]]
                        cand_kw = " ".join(words[:3]) if words else target_post["title"]

                    clean_kw_anchor = re.sub(r'[\'"]', '', cand_kw).strip()
                    target_slug = target_post["slug"]

                    # Attempt natural keyword replacement inside paragraph text if phrase or keyword appears
                    replaced = False
                    # Search for case-insensitive occurrence of candidate keyword in paragraph (ensure not already inside a tag)
                    ...
                    if pattern.search(p_text):
                        ...
                        text = text.replace(p_match.group(0), f'<p>{new_p_text}</p>', 1)
                        replaced = True
                    else:
                        # Try primary tag or single core noun
                        first_tag = (target_post.get("tags") or [""])[0]
                    ...
                    # Natural fallback: weave smoothly on concise keyword anchor rather than dumping whole title
                    if not replaced:
                        ...
                        # Insert before last closing period of the paragraph
